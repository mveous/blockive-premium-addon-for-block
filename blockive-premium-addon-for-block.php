 ...
 */
function bpafb_render_pro_active_notice()
{
...
/**
 * Bootstraps the plugin, deferred to `plugins_loaded` so that
 * BPAFB_PRO_ACTIVE - defined unconditionally at the very top of the Pro
 * plugin's main file - is guaranteed to already be defined by now,
 * regardless of which of the two plugins WordPress happened to load first.
 *
 * When Pro is active, this function defines nothing at all: no constants,
 * no classes, no hooks. Pro ships its own copies of BPAFB_PATH/BPAFB_URL/
 ...
 */
function bpafb_bootstrap()
{
	if (defined('BPAFB_PRO_ACTIVE')) {
		add_action('admin_notices', 'bpafb_render_pro_active_notice');
		return;
	}
...
// Bootstrap the plugin (see bpafb_bootstrap() above for why this is deferred).
add_action('plugins_loaded', 'bpafb_bootstrap');
